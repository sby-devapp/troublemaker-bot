...
class SilenceBreaker(Model):
    table_name = "silence_breakers"
    ...
    @classmethod
    def get_random_by_type(cls, breaker_type):
        """Get a random silence breaker of specified type, prioritizing least-used."""
        ...
        if row:
            breaker = cls()
            breaker.load_object_from_row(row)
            
            # Increment used counter
            update_query = f"UPDATE {cls.table_name} SET used = used + 1 WHERE id = ?"
            cursor.execute(update_query, (breaker.id,))
            db.commit()
